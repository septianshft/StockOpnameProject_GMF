          ...
          {!isScanning && !isLoading && cart.length === 0 && !errorMsg && (
            <div className="bg-white p-12 rounded-[2.5rem] shadow-xl shadow-slate-200/50 border border-slate-200 text-center group animate-in fade-in slide-in-from-bottom-4 duration-500">
              ...
              <div className="pt-8 mt-8 border-t border-slate-100">
                <p className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] mb-3">Item not found?</p>
                <button
                  onClick={() => setShowReqModal(true)}
                  className="w-full bg-slate-50 hover:bg-slate-100 text-slate-600 font-bold py-4 px-6 rounded-2xl border-2 border-slate-300 transition-all active:scale-95 flex justify-center items-center gap-2 text-sm stroke-slate-400 hover:stroke-slate-600"
                >
                  <img src="https://img.icons8.com/?size=100&id=MkbXzLXnS7Wn&format=png&color=000000" alt="Request Item" className="w-7 h-7" /> REQUEST ITEM
                </button>
              </div>
            </div>
          )}
